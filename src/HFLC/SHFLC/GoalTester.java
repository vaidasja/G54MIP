...
public class GoalTester {
	...
	public GoalTester() {
		...
		GoalSeeking goalSeeking = new GoalSeeking(-180.0, -60.0, -120.0, 0.0,
				60.0, 180.0, 0.0, 120.0, -30.0, 0.0, 0.0, 30.0, -100.0, 0.0,
				0.0, 100.0, leftWheelLow, leftWheelMedium, leftWheelHigh,
		...
		while (true) {
			...
			double angle = 90 - Math.toDegrees(Math.atan(Math.abs(robot.getX())
					/ Math.abs(robot.getY())));
			// System.out.println("Left Front: "+ leftFrontSonar +
			// " Left Back : " + leftBackSonar + " Right front: " +
			// rightFrontSonar + " Right back: " + rightBackSonar + " Front: " +
			// frontMiddleSonar + " Front Left: " + frontLeftSonar +
			// "Front Right " + frontRightSonar);
			// double bearing =
			// 90-(180/Math.PI)*Math.atan2(GOAL_Y-robot.getY(),GOAL_X-robot.getX());
			...
			// double leftLeft =
			// leftWallFollowing.getRulebase().evaluate(0).get(leftWheelVelocity);
			// double leftRight =
			// leftWallFollowing.getRulebase().evaluate(0).get(rightWheelVelocity);
			// double rightLeft =
			// rightWallFollowing.getRulebase().evaluate(0).get(leftWheelVelocity);
			// double rightRight =
			// rightWallFollowing.getRulebase().evaluate(0).get(rightWheelVelocity);
			double leftOutput = goalSeeking.getRulebase().evaluate(0)
					.get(leftWheelVelocity);
			double rightOutput = goalSeeking.getRulebase().evaluate(0)
